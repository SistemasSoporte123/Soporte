...
// Obtener la cantidad de registros por área
if($_SESSION['username']=='Alexis_Aspel'){
    $sql_soporte = "SELECT area, COUNT(*) as count FROM aspel_soporte GROUP BY area";
    $result_soporte = $conn->query($sql_soporte); 
}else{
    $sql_soporte = "SELECT area, COUNT(*) as count FROM soporte GROUP BY area";
    $result_soporte = $conn->query($sql_soporte);    
}
...
<html>
<head>
    <title>Gráfica de Soporte por Área</title>
...
<body>
    ...
    <canvas id="soporteChart" width="400" height="200"></canvas>

    <script>
    var ctxSoporte = document.getElementById('soporteChart').getContext('2d');
    var soporteChart = new Chart(ctxSoporte, {
        type: 'bar',
        data: {
            labels: areas,
            datasets: [{
                label: 'Cantidad de Registros por Área',
                data: counts,
