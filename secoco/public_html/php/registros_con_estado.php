...
// Consulta SQL para obtener los datos de la tabla soporte donde estado no es nulo y ordenar por id en orden descendente
if($_SESSION['username']=='Alexis_Aspel'){
    $sql_soporte = "SELECT * FROM aspel_soporte WHERE estado IS NOT NULL ORDER BY id DESC";
    $result_soporte = $conn->query($sql_soporte);
}else{
    $sql_soporte = "SELECT * FROM soporte WHERE estado IS NOT NULL ORDER BY id DESC";
    $result_soporte = $conn->query($sql_soporte);
}
// Cerrar conexión
$conn->close();
...
<body>
    <main>
        ...
        <div class="container mt-4">
            <h1>Registros de Soporte con Estado</h1>
                ...
                <tbody>
                    ...
                    if ($result_soporte->num_rows > 0) {
                        while ($row = $result_soporte->fetch_assoc()) {
                            echo "<tr>";
                            echo "<td> RDS" . htmlspecialchars($row['id']) . "</td>";
                            echo "<td>" . htmlspecialchars($row['nombre']) . "</td>";
                            echo "<td>" . htmlspecialchars($row['h_inicio']) . "</td>";
                            echo "<td>" . htmlspecialchars($row['estado']) . "</td>";
                            echo "<td>";
                            echo "<a href='imprimir_registro.php?id=" . $row['id'] . "' class='btn btn-success ms-2'>Imprimir</a>";
                            echo "</td>";
                            echo "</tr>";
                        }
                    } else {
